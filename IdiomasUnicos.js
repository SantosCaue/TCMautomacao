...
for (j = 0; j < Object.keys(lista).length; j++) {
    var valores = (lista[Object.keys(lista)[j]].idioma_en).replace(" and ", ",");
    var dividido = valores.split(",");
    for (v = 0; v < dividido.length; v++) {
        dividido[v] = dividido[v].trim();
    }
    for (h = 0; h < dividido.length; h++) {
        var unico = true;
        for (p = 0; p < idiomas.length; p++) {
            if (dividido[h] == idiomas[p]) {
                unico = false;
            }
        }
        if (unico) {
            idiomas.push(dividido[h]);
        }
    }
}
console.log(idiomas.length - 1);
for(t = 41; t< idiomas.length; t++){
console.log(`.replace("${idiomas[t]}", "")`);
}