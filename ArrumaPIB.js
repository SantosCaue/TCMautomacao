...
function formatarNumeroComSeparador(numero) {
    // Converte o número para uma string e remove espaços em branco no início e no fim
    numero = String(numero).trim();
  
    // Divide a string em partes de até 3 dígitos a partir do final
    const partes = [];
    while (numero.length > 0) {
      partes.unshift(numero.slice(-3));
      numero = numero.slice(0, -3);
    }
  
    // Junta as partes com um ponto como separador
    return partes.join('.');
  }
...
for (j = 0; j < Object.keys(lista).length; j++) {
    var gdp = lista[Object.keys(lista)[j]].pib;
    gdp = gdp.split(" ");
    if(gdp.length > 3){
        gdp[0] = gdp[0].replace(",", "").replace("$", "");
        if(gdp[1] == "million"){
           gdp[0] = parseFloat(gdp[0]) * 1000000;
        }else{
            gdp[0] = parseFloat(gdp[0]) * 1000000000;
        }
        
        lista[Object.keys(lista)[j]].pib = `$${formatarNumeroComSeparador(gdp[0])} ${gdp[2]} ${gdp[3]}`
    }else{
        gdp[0] = gdp[0].replaceAll(",",".")
        lista[Object.keys(lista)[j]].pib = `${gdp[0]} ${gdp[1]} ${gdp[2]}`
    }
    lista[Object.keys(lista)[j]].area = lista[Object.keys(lista)[j]].area.replaceAll(",", ".");
    lista[Object.keys(lista)[j]].populacao = lista[Object.keys(lista)[j]].populacao.replaceAll(",", ".");

}

fs.writeFileSync('ListaPIBCerto.json', JSON.stringify(lista), "utf-8")